...
let percentHumanFemales = Float(humanFemalesCount)/Float(humanOfficers.count)
...
let firstHumanOfficers = firstOfficers.keys.filter{humanOfficers.contains($0)}.count
print(firstHumanOfficers)



// Q3: Get a list of females sorted by name
let femalesSorted = femaleOfficers.sorted(by: {$0 < $1})
print(femalesSorted)
...
let firstVulcanOfficers = allFirstOfficers.filter{vulcanOfficers.contains($0.name)}
